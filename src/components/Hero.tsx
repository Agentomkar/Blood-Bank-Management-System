  ...
  const drops = useMemo(
    () =>
      Array.from({ length: 34 }, (_, i) => ({
        ...
        delay: seededUnit(i + 101) * 10,
        duration: 8 + seededUnit(i + 201) * 15,
        size: 3 + seededUnit(i + 301) * 14,
        opacity: 0.04 + seededUnit(i + 401) * 0.12,
      })),
    []
  );
      ...
      >
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 items-end gap-8 lg:grid-cols-[minmax(0,0.98fr)_minmax(280px,0.48fr)]">
          <div className="hero-glass-panel rounded-[2rem] border border-white/10 bg-black/32 p-6 text-left shadow-2xl shadow-black/45 backdrop-blur-2xl md:p-8 lg:p-10">
            ...
            >
              <span className="hero-line block">Save Lives Through</span>
              <span className="hero-line block gradient-text">
                <TypewriterEffect text="Smart Blood Management" delay={800} />
              </span>
            </h1>
            ...
            >
              A cinematic, intelligent blood bank platform with real-time inventory,
              AI-powered matching, emergency coordination, and premium hospital-grade
              analytics.
            </motion.p>
